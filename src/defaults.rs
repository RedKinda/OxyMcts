...
use crate::tree::{NodeId, NodeMutRef, Tree};
    ...
    Reward: Div + ToPrimitive + Add + Zero,
{
    ...
    pub fn expand(
        mut node_to_expand: NodeMutRef<LazyMctsNode<State, Reward, A>>,
        root_state: State,
    ) -> (NodeId, State) {
...
impl<State, EV, A, Reward> LazyTreePolicy<State, EV, A, Reward>
    for DefaultLazyTreePolicy<State, EV, A, Reward>
where
    State: GameTrait,
    ...
    A: Clone + Default,
{
    ...
    fn best_child(
        ...
        let best =
            parent_node.get_best_child(|child| EV::eval_child(child, turn, n_visits, eval_args));
...
impl<
        ...
        Reward: Clone + Div + Zero + ToPrimitive + Add,
    > Evaluator<State, Reward, AdditionalInfo> for DefaultUctEvaluator
{
    type Args = f64;
    type EvalResult = Nat;
